...
So now we have the old-age threshold for every year. We also have the population data, single-year age groups, for every year. So in order to get the proportion of the population over the old age-threshold, we now need to interpolate the population. E.g. there are 30,000 people aged 62 but not yet 63. How many people are aged 62.3? Again, this interpolation could be done linearly, but since we have information on the populaition at ages 61 and 63 etc, it makes sense to use that in the calculation and use splines again. 

Hmm, OK, there is a problem here, because the interpolation of the five year time periods makes the final proportion all jaggedy, but it kind of works..
